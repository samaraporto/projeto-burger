...
async function main() {
  ...
  nome.addEventListener("blur", () => {
    ...
    if (tamanhoNome > 10 && partesNome.length > 1 && regex.test(nome.value)) {
      nomeValido = true;
      infos.textContent = "";
      if (nome.style.border != "2px solid #90EE90")
        nome.style.border = "2px solid #90EE90";
    } else {
  ...
  confirmacaoSenha.addEventListener("blur", () => {
    ...
    if (
      confirmacaoSenhaValor === senhaValor &&
      confirmacaoSenhaValor.length > 7 &&
      !/\s/.test(confirmacaoSenhaValor) &&
      /[0-9!@#$%^&*]/.test(confirmacaoSenhaValor)
    ) {
      confirmacaoSenhaValida = true;
      infos.textContent = "";
      if (confirmacaoSenha.style.border !== "2px solid #90EE90")
        confirmacaoSenha.style.border = "2px solid #90EE90"; // Validação correta
    } else {
      if (confirmacaoSenha.style.border !== "2px solid red")
        confirmacaoSenha.style.border = "2px solid red";
      if (confirmacaoSenhaValor !== senhaValor)
        infos.textContent = "A confirmação de senha deve corresponder à senha.";
      if (confirmacaoSenhaValor.length <= 7)
        infos.textContent = "A senha deve ter mais de 7 caracteres.";
      if (/\s/.test(confirmacaoSenhaValor))
        infos.textContent = "A senha não deve conter espaços em branco.";
      if (!/[0-9!@#$%^&*]/.test(confirmacaoSenhaValor))
        infos.textContent =
          "A senha deve conter pelo menos um caractere especial ou numérico.";
    }
  });
  ...
  numeroDeTelefone.addEventListener("blur", () => {
    ...
    if (telefone.length > 10) {
      ...
      infos.textContent = "O telefone deve ter mais de 10 dígitos.";
    }
  });
  ...
  document.querySelector("form").addEventListener("submit", async (event) => {
    ...
    if (senha.value !== confirmacaoSenha.value) {
      infos.textContent = "as senhas nao coincidem";
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`${data.message || "erro ao criar user"}`);
      }
    } catch (error) {
